...
def generar_pdf_reporte(df_local):
    # Crear el documento PDF en orientación Horizontal (Landscape) 
    # Es mejor usar Horizontal para que quepan bien todas las columnas en la hoja
    pdf = FPDF(orientation="L", unit="mm", format="A4")
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.add_page()
    
    # 1. TÍTULO CORPORATIVO (Mismos colores de tu App)
    pdf.set_font("Helvetica", "B", 16)
    pdf.set_text_color(12, 35, 64) # Color #0C2340
    pdf.cell(0, 10, txt="PLANTA METALES Y MAQUINADOS", ln=True, align="C")
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, txt="MATRIZ DE COMUNICACIÓN EFECTIVA - REPORTE DE PENDIENTES", ln=True, align="C")
    pdf.set_font("Helvetica", "I", 9)
    pdf.set_text_color(100, 100, 100)
    pdf.cell(0, 5, txt=f"Reporte generado en la nube el: {datetime.now().strftime('%d-%b-%y %H:%M')}", ln=True, align="C")
    pdf.ln(8)
    
    # 2. SECCIÓN DE MINI DASHBOARD (MÉTRICAS)
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(12, 35, 64)
    pdf.cell(0, 8, txt="1. RESUMEN DEL DASHBOARD INTERACTIVO", ln=True)
    pdf.set_draw_color(12, 35, 64)
    pdf.line(10, pdf.get_y(), 285, pdf.get_y()) # Línea divisoria larga para hoja horizontal
    pdf.ln(4)
    
    # Cálculos rápidos en base al filtro de pantalla
    total = len(df_local)
    terminadas = len(df_local[df_local["% Avance"].astype(int) == 100])
    pendientes_semana = len(df_local[df_local["% Avance"].astype(int) < 100])
    avance_prom = df_local["% Avance"].astype(int).mean() if total > 0 else 0
    
    # Dibujar las tarjetas del Dashboard en el PDF
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(0, 0, 0)
    pdf.cell(65, 8, txt=f" Total Actividades: {total}", border=1)
    pdf.cell(65, 8, txt=f" Tareas Terminadas: {terminadas}", border=1)
    pdf.cell(65, 8, txt=f" Tareas Pendientes: {pendientes_semana}", border=1)
    pdf.cell(80, 8, txt=f" Eficiencia de Avance Promedio: {avance_prom:.1f}%", border=1, ln=True)
    pdf.ln(10)
    
    # 3. SECCIÓN DETALLE DE ACTIVIDADES PENDIENTES
    pdf.set_font("Helvetica", "B", 12)
    pdf.set_text_color(12, 35, 64)
    pdf.cell(0, 8, txt="2. LISTADO DETALLADO DE ACTIVIDADES PENDIENTES DE LA SEMANA", ln=True)
    pdf.line(10, pdf.get_y(), 285, pdf.get_y())
    pdf.ln(4)
    
    # Filtrar solo pendientes (Menos de 100%)
    df_solo_pendientes = df_local[df_local["% Avance"].astype(int) < 100]
    
    if df_solo_pendientes.empty:
        pdf.set_font("Helvetica", "I", 11)
        pdf.set_text_color(150, 150, 150)
        pdf.cell(0, 10, txt="¡Felicidades! No hay actividades pendientes registradas bajo este filtro.", ln=True)
    else:
        # ENCABEZADOS DE LA TABLA (Ancho total: 275mm para diseño horizontal)
        pdf.set_font("Helvetica", "B", 9)
        pdf.set_fill_color(207, 216, 220) # Color #CFD8DC de tu sidebar
        pdf.set_text_color(12, 35, 64)
        
        pdf.cell(12, 7, txt="No", border=1, fill=True, align="C")
        pdf.cell(30, 7, txt="Área", border=1, fill=True)
        pdf.cell(40, 7, txt="Responsable", border=1, fill=True)
        pdf.cell(133, 7, txt="Descripción del Pendiente", border=1, fill=True)
        pdf.cell(20, 7, txt="Avance", border=1, fill=True, align="C")
        ...
        for _, fila in df_solo_pendientes.iterrows():
            # Limitar la descripción para que no se desborde de la celda
            texto_desc = str(fila["Descripcion"])
            if len(texto_desc) > 85:
                texto_desc = texto_desc[:82] + "..."
            
            # Formato visual de celda amarilla suave para pendientes (similar a tu semáforo)
            pdf.set_fill_color(255, 243, 205) # Color #FFF3CD
            
            pdf.cell(12, 6, txt=str(fila["No"]), border=1, align="C")
            pdf.cell(30, 6, txt=str(fila["Area"])[:18], border=1)
            pdf.cell(40, 6, txt=str(fila["Responsable"])[:24], border=1)
            pdf.cell(133, 6, txt=texto_desc, border=1)
            pdf.cell(20, 6, txt=f" {fila['% Avance']}%", border=1, fill=True, align="C")
            pdf.cell(40, 6, txt=str(fila["Fecha Compromiso"]), border=1, ln=True)
            
    # Convertir el PDF generado a un arreglo de bytes para Streamlit Cloud
    return pdf.output()

    # Crear el PDF usando los datos que están filtrados actualmente en la pantalla
    try:
        pdf_bytes = generar_pdf_reporte(df_mostrar)
        st.download_button(
            label="📄 Descargar Reporte en PDF (Pendientes)",
            data=bytes(pdf_bytes),
            file_name=f"Reporte_Pendientes_MCE_{datetime.now().strftime('%d-%b-%y')}.pdf",
            mime="application/pdf",
            key="btn_pdf_cloud_mce"
    )
    except Exception as e_pdf:
        st.error(f"Error temporal al estructurar el PDF: {e_pdf}")
